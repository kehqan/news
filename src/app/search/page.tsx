...
// Server-rendered keyword search across all feeds. Zero client JS.
...
export default async function SearchPage({ searchParams }: PageProps) {
  ...
  if (query.length >= 2) {
    // Fetch all sections in parallel and search across them
    const sections = ["latest", "iran", "world", "economy", "culture"];
    const feeds = await Promise.all(sections.map((s) => fetchFeed(s)));
